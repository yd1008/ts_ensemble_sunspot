...
### Predict
    ...
    fig.savefig(root_dir + '/figs/gru_future_pred.png')
    plt.close(fig)
...
### Check MSE, MAE
    ...
    ax.axhline(y=0)
    ax.legend(loc="upper right")
    fig.savefig(root_dir + '/figs/gru_future_inversed_pred.png')
    plt.close(fig)
### Save model result


    future_result_df = pd.DataFrame(future_result)
    future_result_df.to_csv(root_dir + '/gru_future_prediction.csv')
